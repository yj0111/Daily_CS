...
> 프레임워크는 구조를 설계한 것이기 때문에 개발자가 프레임워크의 구조에 맞게 프로그래밍 해야하지만<br>
...
질문. DB에서 인덱스를 잘 사용하면 어떤 장점이 있을까요?<br>
답변 >
...
질문. 객체지향이 무엇인지 설명해보세요 <br>
답변 > 프로그래밍에서 필요한 데이터를 추상화시켜서 상태(속성, 어트리뷰트)와 행위(메서드)를 가진 객체로 만들고,
그 객체간의 상호 작용을 통해 로직을 구성하는 방법을 의미한다.

장점) 1. 다른 클래스를 가져와 사용할 수 있고, 상속받을 수 있어 코드의 재사용성 증가
2. 자주 사용되는 로직을 라이브러리로 만들어두면 계속해서 사용할 수 있어 신뢰성을 확보 가능
3. 클래스 단위로 모듈화가 가능하여, 대형 프로젝트에 적합.(위와 비슷한 의미)
4. 객체 단위로 코드가 나눠져 작성되기 때문에 디버깅이 쉽고 유지보수가 용이함.

단점) 1. 처리 속도가 상대적으로 느림
2. 객체가 많으면 용량이 커짐
3. 설계 시 많은 노력과 시간이 소요

### N+1 문제 <br>
답변. 1:N 관계에서 1번 쿼리를 날렸는데 추가적으로 N번의 쿼리가 발생하는 상황을 말하며,  Fetch Join과 @EntityGraph을 사용하여 해결하는 방법이 있습니다.